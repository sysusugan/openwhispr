...
function normalizeDictionaryAliases(aliases = [], dictionary = []) {
...
function isLikelyDictionaryMiss(candidate, target) {
  ...
  // Some ASR misses are phonetically close but visually farther, e.g. Antibus -> EntVerse.
...
function applyDictionaryCorrections(text, options = {}) {
  ...
  corrected = corrected.replace(/(?<![\p{L}\p{N}_])([A-Za-z][A-Za-z0-9_-]*)(?![\p{L}\p{N}_])/gu, (match) => {
    for (const target of dictionary) {
